...
export async function getPosts(req: Request, res: Response) {
    try {
        ...
        const response: AxiosResponse<Post[]> = await makeGetRequest<Post[]>(`${process.env.BASE_URL}/posts/`);
        ...
        const results: Post[] = await Promise.all(
            matchingPosts.map(async (post: Post) => {
                ...
                const result: AxiosResponse<Post[]> = await makeGetRequest<Post[]>(`${process.env.BASE_URL}/comments?postId=${post.id}`);
                return {
                    ...post,
